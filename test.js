...
function Particle(x, y, distance) {
  ...
  this.opacity =  (Math.random()*8 + 3)/10;
  this.distance = (1.5/this.opacity)*distance;
  ...
  this.draw = function() {
    ctx.fillStyle = "rgba(0, 0, 0, 0.6," + this.opacity + ")";
    ctx.beginPath();
    ctx.arc(this.position.x, this.position.y, this.radius, 0, Math.PI*20, false);
    ctx.fill();
    ctx.closePath();
  }
